...
func (s *storage) SaveBlock(ctx context.Context, num *big.Int) (int64, error) {
	const op = "storage.SaveBlock"

	var blockID int64
	if err := s.db.QueryRowContext(ctx, `select id from nft.forge_block where block_number = $1`, num.Int64()).Scan(&blockID); err != nil {
